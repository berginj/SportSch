  ...
  return (
    <div className="card coachLinks">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="coachLinks__stat coachLinks__stat--info">
          <div className="text-2xl font-bold coachLinks__statValue">{teams.length}</div>
          <div className="text-sm coachLinks__statLabel">Total Teams</div>
        </div>
        <div className="coachLinks__stat coachLinks__stat--ok">
          <div className="text-2xl font-bold coachLinks__statValue">{completeCount}</div>
          <div className="text-sm coachLinks__statLabel">Onboarding Complete</div>
        </div>
        <div className="coachLinks__stat coachLinks__stat--warn">
          <div className="text-2xl font-bold coachLinks__statValue">{incompleteCount}</div>
          <div className="text-sm coachLinks__statLabel">Pending Setup</div>
        </div>
      </div>

      <div className="coachLinks__toolbar flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-4 mb-6 pb-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto sm:ml-auto">
          <button className="btn btn--sm" onClick={copyAllLinks} disabled={filteredTeams.length === 0}>
            Copy All as CSV
          </button>
          <button className="btn btn--sm btn--primary" onClick={downloadCSV} disabled={filteredTeams.length === 0}>
            Download CSV
          </button>
        </div>
      </div>
      ...
      {filteredTeams.length === 0 ? (
        <div className="text-center py-8 muted">No teams match the current filters</div>
      ) : (
        <div className="space-y-3">
          {filteredTeams.map((team) => {
            ...
            return (
              <div key={`${team.division}-${team.teamId}`} className="coachLinks__teamCard p-4 rounded-lg">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {team.onboardingComplete && <span className="statusBadge status-confirmed">Complete</span>}
                    </div>
                    ...
                    {hasContact ? (
                      ...
                      <div className="text-sm muted italic mb-2">No contact information</div>
                    )}
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mt-3">
                      <input
                        ...
                        value={link}
                        readOnly
                        className="coachLinks__linkInput flex-1 min-w-0 text-xs sm:text-sm font-mono rounded px-2 py-1.5"
      ...
      <div className="mt-6 p-4 callout callout--info">
        <div className="font-semibold mb-2">How to Send Links to Coaches</div>
