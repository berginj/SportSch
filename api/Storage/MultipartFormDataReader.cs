using System.Text;
using System.Text.RegularExpressions;
using Microsoft.Azure.Functions.Worker.Http;

namespace GameSwap.Functions.Storage;

public sealed record MultipartFormFile(string Name, string FileName, string ContentType, byte[] Content);

public sealed class MultipartFormDataPayload
{
    public Dictionary<string, string> Fields { get; } = new(StringComparer.OrdinalIgnoreCase);
    public List<MultipartFormFile> Files { get; } = new();
}

public static class MultipartFormDataReader
{
    public static async Task<MultipartFormDataPayload> ReadAsync(HttpRequestData req)
    {
        using var ms = new MemoryStream();
        await req.Body.CopyToAsync(ms);
        return Read(ms.ToArray(), GetHeader(req, "Content-Type"));
    }

    public static MultipartFormDataPayload Read(byte[] body, string? contentType)
    {
        var payload = new MultipartFormDataPayload();
        if (body.Length == 0) return payload;

        var boundary = ExtractBoundary(contentType ?? "");
        if (string.IsNullOrWhiteSpace(boundary))
        {
            var firstLineEnd = IndexOf(body, Encoding.UTF8.GetBytes("\r\n"), 0);
            if (firstLineEnd > 2)
            {
                var firstLine = Encoding.UTF8.GetString(body, 0, firstLineEnd).Trim();
                if (firstLine.StartsWith("--", StringComparison.Ordinal) && firstLine.Length > 4)
                    boundary = firstLine[2..];
            }
        }

        if (string.IsNullOrWhiteSpace(boundary)) return payload;

        var boundaryBytes = Encoding.UTF8.GetBytes("--" + boundary);
        var endBoundaryBytes = Encoding.UTF8.GetBytes("--" + boundary + "--");
        var parts = SplitMultipart(body, boundaryBytes, endBoundaryBytes);

        foreach (var part in parts)
        {
            var headerEnd = IndexOf(part, Encoding.UTF8.GetBytes("\r\n\r\n"), 0);
            if (headerEnd < 0) continue;

            var headerText = Encoding.UTF8.GetString(part, 0, headerEnd);
            var contentStart = headerEnd + 4;
            var content = part.AsSpan(contentStart).ToArray();
            if (content.Length >= 2 && content[^2] == (byte)'\r' && content[^1] == (byte)'\n')
                content = content[..^2];

            var disposition = GetContentDisposition(headerText);
            if (disposition is null || string.IsNullOrWhiteSpace(disposition.Value.name)) continue;

            if (!string.IsNullOrWhiteSpace(disposition.Value.filename))
            {
                payload.Files.Add(new MultipartFormFile(
                    disposition.Value.name,
                    disposition.Value.filename,
                    GetContentType(headerText),
                    content));
                continue;
            }

            payload.Fields[disposition.Value.name] = Encoding.UTF8.GetString(content).Trim();
        }

        return payload;
    }

    private static string GetHeader(HttpRequestData req, string name)
        => req.Headers.TryGetValues(name, out var vals) ? (vals.FirstOrDefault() ?? "") : "";

    private static string ExtractBoundary(string contentType)
    {
        if (string.IsNullOrWhiteSpace(contentType)) return "";
        var match = Regex.Match(contentType, @"boundary=(?:""(?<b>[^""]+)""|(?<b>[^;]+))", RegexOptions.IgnoreCase);
        return match.Success ? match.Groups["b"].Value.Trim() : "";
    }

    private static (string? name, string? filename)? GetContentDisposition(string headers)
    {
        var lines = headers.Split(new[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries);
        foreach (var line in lines)
        {
            if (!line.StartsWith("Content-Disposition:", StringComparison.OrdinalIgnoreCase)) continue;
            var name = Regex.Match(line, @"name=(?:""(?<n>[^""]+)""|(?<n>[^;]+))", RegexOptions.IgnoreCase);
            var file = Regex.Match(line, @"filename=(?:""(?<f>[^""]+)""|(?<f>[^;]+))", RegexOptions.IgnoreCase);
            return (name.Success ? name.Groups["n"].Value : null, file.Success ? file.Groups["f"].Value : null);
        }
        return null;
    }

    private static string GetContentType(string headers)
    {
        var lines = headers.Split(new[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries);
        foreach (var line in lines)
        {
            if (!line.StartsWith("Content-Type:", StringComparison.OrdinalIgnoreCase)) continue;
            return line["Content-Type:".Length..].Trim();
        }
        return "application/octet-stream";
    }

    private static List<byte[]> SplitMultipart(byte[] body, byte[] boundary, byte[] endBoundary)
    {
        var parts = new List<byte[]>();
        var pos = IndexOf(body, boundary, 0);
        if (pos < 0) return parts;

        while (pos >= 0)
        {
            pos += boundary.Length;
            if (pos + 2 <= body.Length && body[pos] == (byte)'\r' && body[pos + 1] == (byte)'\n') pos += 2;

            if (pos - boundary.Length >= 0 && StartsWithAt(body, pos - boundary.Length, endBoundary))
                break;

            var next = IndexOf(body, boundary, pos);
            var nextEnd = IndexOf(body, endBoundary, pos);
            var cut = (nextEnd >= 0 && (next < 0 || nextEnd < next)) ? nextEnd : next;
            if (cut < 0) break;

            var len = cut - pos;
            if (len > 0)
            {
                var part = new byte[len];
                Buffer.BlockCopy(body, pos, part, 0, len);
                parts.Add(part);
            }

            pos = cut;
        }

        return parts;
    }

    private static bool StartsWithAt(byte[] haystack, int start, byte[] needle)
    {
        if (start < 0 || start + needle.Length > haystack.Length) return false;
        for (var i = 0; i < needle.Length; i++)
        {
            if (haystack[start + i] != needle[i]) return false;
        }
        return true;
    }

    private static int IndexOf(byte[] haystack, byte[] needle, int start)
    {
        if (needle.Length == 0) return -1;
        for (var i = start; i <= haystack.Length - needle.Length; i++)
        {
            var match = true;
            for (var j = 0; j < needle.Length; j++)
            {
                if (haystack[i + j] != needle[j])
                {
                    match = false;
                    break;
                }
            }
            if (match) return i;
        }
        return -1;
    }
}
